<template>
    <div>
        <div
            ...
            <p class="mb-3 font-normal text-gray-700 dark:text-gray-400">{{ comment.content }}</p>
            <p class="mt-3 font-normal text-xs text-gray-300 dark:text-gray-300">Posté le: {{ new
                    Date(comment.createdAt).toLocaleString()
            }}</p>
            <p class="font-normal text-xs text-gray-300 dark:text-gray-300">Par: {{ comment.createBy }}</p>
            <div class="react-icon">
                <div class="cursor-pointer"
                    v-if="loggedInUser.user.isAdmin == true || comment.createBy == loggedInUser.user.pseudo"
                    v-on:click="openEdit">
...
export default {
    ...
    methods: {
        ...
        async deletePost() {
            try {
                await this.$axios.post('post/deletecomment', {
                    commentId: this.comment.id,
                });

                $nuxt.$emit('deletingComment');
                this.showEdit = false;
            }
            catch (e) {
                console.log(e)
